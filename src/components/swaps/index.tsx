...
export default function Swaps({ initialSell, initialBuy }: SwapsProps) {
  const router = useRouter();
  const { sell, buy } = router.query;
  ...
  useEffect(() => {
    const routeFetch = () => {
      let sellIsBase: boolean = false;

      // Eğer dışarıdan gelen initialSell ve initialBuy varsa, bunları kullan
      const sellToken = initialSell || sell;
      const buyToken = initialBuy || buy;

      if (sellToken && TokenList) {
        if (sellToken === buyToken) {
          setTokenIn(baseTokens[0]);
          return;
        }
        const matchedTokenIn = TokenList.find(
          (token) => token.contractAddress === sellToken
        );
        if (matchedTokenIn) {
          sellIsBase = matchedTokenIn?.isBase;
          setTokenIn(matchedTokenIn);
        } else {
          setTokenIn(baseTokens[0]);
          sellIsBase = true;
        }
      ...
      if (buyToken) {
        if (sellIsBase) {
          const matchedTokenOut = quoteTokens.find(
            (token) => token.contractAddress === buyToken
          );
          setTokenOut(matchedTokenOut || undefined);
        } else {
          const matchedTokenOut = baseTokens.find(
            (token) => token.contractAddress === buyToken
          );
          setTokenOut(matchedTokenOut || undefined);
        }
    ...
    routeFetch();
    setMounted(true);
  }, [baseTokens, sell, buy, initialSell, initialBuy]);
  ...
  const handleSelectTokenIn = (token: TToken) => {
    ...
    router.push({
      ...
      query: {
        ...router.query,
        sell: token.contractAddress,
        buy: tokenOut ? tokenOut.contractAddress : undefined,
      },
    });
  };
  ...
  const handleSelectTokenOut = (token: TToken) => {
    ...
    router.push({
      ...
      query: {
        ...router.query,
        sell: tokenIn ? tokenIn.contractAddress : undefined,
        buy: token.contractAddress,
      },
    });
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col w-[450px] sm:w-[92.5%] sm:overflow-hidden">
        <div className="flex items-center w-full justify-between">
          <div className="flex gap-3 items-center">
            <Button
              ...
              className={`${
                selectedTab === "swap"
                  ? "text-white bg-upper-background"
                  : "text-text-passive bg-transparent"
              }  font-bold text-md`}
            >
              Swap
            </Button>
